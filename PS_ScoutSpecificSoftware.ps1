...
Load Function: ". .\Get-RemoteProgram.ps1" <--This must be done 1st!
...
Description:
Will genrate a list of installed programs on all the hosts in serverlist.txt
...
.EXAMPLE
Get-Content -Path C:\Temp\servers.txt |  Get-RemoteProgram -IncludeProgram 'ProgramName' -Property DisplayVersion | Export-Csv C:\Temp\Server_List_Output.csv

Description
Will retrieve the the version of 'ProgramName' on the hosts listed in C:\Temp\servers.txt and output it to a .csv
#>
...
# Search Criteria:
# To use a static .txt file use: Get-Content -Path C:\PathToYour.txt
# AD OU Search: Get-ADComputer -Filter * -SearchBase 'OU=Servers,DC=company,DC=com' -Properties Name | Select-Object -ExpandProperty Name
# Drop either of these just before: " | For-EachObject {" below:

Get-Content -Path C:\Temp\hosts.txt | ForEach-Object {    
    try {
        $computerName = $_
...
# Give your .csv file a path to save to:

} | Export-Csv -Path C:\Temp\SoftwareScoutManifest.csv -NoTypeInformation -Append